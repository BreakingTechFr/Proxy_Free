...
class Downloadproxies():
    ...
    def get_extra(self):
        for q in range(20):
            ...
            # Vérification si la réponse est vide ou non valide
            if self.r.status_code == 200 and self.r.text.strip():
                try:
                    self.json_result = json.loads(self.r.text)
                except json.JSONDecodeError:
                    print(f"Erreur de décodage JSON pour la date {self.day}. Réponse: {self.r.text}")
                    continue  # Passer à l'itération suivante en cas d'erreur JSON
            else:
                print(f"Aucune donnée reçue pour la date {self.day}. Réponse: {self.r.text}")
                continue
